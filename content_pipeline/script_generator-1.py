...
# NOTE: this catches RUNS of 2+ symbols. It intentionally does NOT include $ or %
# (this is a finance channel — "$5 ٹریلین" / "10%" are legitimate) and it does NOT
# include a bare single '-' or ':' (legitimate in date ranges like "1929-1930" and
# in the rare timestamp mention). Kept for backward-compat / corrupted-scene detection.
GARBAGE_PATTERN = re.compile(r"[&!@#$%^*+=~`|<>{}\[\]\\/]{2,}")

# Characters that should NEVER appear in spoken Urdu narration, even as a single
# occurrence — unlike GARBAGE_PATTERN above, this has no {2,} threshold, which is
# exactly why a lone stray '@' or '/' was previously slipping through untouched.
# $ and % are deliberately excluded (legitimate finance content).
_NEVER_LEGIT_CHARS = re.compile(r"[&@#^*+=~`|<>{}\[\]\\]")

# URL-scheme leftovers / repeated slashes and colons (e.g. "://", "//") are noise,
# but a single "-" or ":" is left alone since those are legitimate in dates,
# ranges, and occasional time references.
...
def _sanitize_narration(text: str) -> str:
    ...
    # Remove single-occurrence junk characters that never legitimately appear in
    # narration (this is the fix for stray "@" / "/" / "\" etc. that the run-based
    # GARBAGE_PATTERN above was silently letting through)
    text = _NEVER_LEGIT_CHARS.sub(" ", text)

    # Remove URL-scheme leftovers and repeated slash/colon/hyphen noise
...
def _call_groq(client: Groq, system_prompt: str, user_content: str, max_tokens: int, depth: int = 0) -> dict:
    ...
    if finish_reason == "length":
        print(
            ...
            f"finishing — output is truncated. This is a likely cause of short/cut-off narration."
        )
        ...
        try:
            ...
            if finish_reason == "length":
                print(
                    ...
                    f"before finishing — output is truncated. This is a likely cause of short/cut-off "
                    f"narration. Consider raising SAMBANOVA_MIN_TOKENS_FLOOR."
                )
...
def _generate_scenes_chunk(
    ...
    if actual_words < word_budget * MIN_ACCEPTABLE_RATIO:
        # Retrying with the SAME max_tokens as the attempt that came up short is
        # pointless if the shortfall was caused by hitting that ceiling mid-JSON
        # (truncated output) — it'll likely truncate again in the same place.
        # Give it real headroom this time.
